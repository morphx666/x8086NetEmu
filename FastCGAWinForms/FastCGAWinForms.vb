...
Public Class FastCGAWinForms
    ...
    Private Class CGAChar
        ...
        Private mBitmap As UnmanagedCachedBitmap
        ...
        Public Sub Paint(ug As UnmanagedGraphics, p As Point)
            ug.DrawCachedBitmap(mBitmap, p.X, p.Y)
        End Sub

        Public Sub Render(ug As UnmanagedGraphics)
            If mBitmap Is Nothing Then
                ...
                Using g As Graphics = Graphics.FromImage(bmp)
                    ...
                    Dim tmpBitmap As UnmanagedBitmap = UnmanagedBitmap.FromHicon(bmp.GetHicon())
                    mBitmap = New UnmanagedCachedBitmap(tmpBitmap, ug)
                    tmpBitmap.Dispose()
                End Using
            End If
        End Sub
    ...
    Private Sub RenderText(g As Graphics)
        ...
        Using ug As New UnmanagedGraphics(hdc)
            ug.ScaleTransform(scale.Width, scale.Height)

            For address As Integer = StartTextVideoAddress To EndTextVideoAddress Step 2
                ' Idealy, we should use cpu.RAM as it's safer, but using cpu.Memory should be faster
                b0 = CPU.Memory(address)
                b1 = CPU.Memory(address + 1)

                If useCGAFont Then
                    RenderChar(b0, ug, brushCache(b1.LowNib()), brushCache(b1.HighNib()), r.Location)
                Else
    ...
    Private Sub RenderChar(c As Integer, ug As UnmanagedGraphics, fb As SolidBrush, bb As SolidBrush, p As Point)
